...
public class Glider {
    ...
    public static void promptGlider() throws Exception {
        ...
        System.out.println("Enter Glider Name:");
        String selection = new Scanner(System.in).nextLine().replace(" ", "%20").replace(".uasset", "");
        start = System.currentTimeMillis();
        String skinSelectionFormat = String.format("https://benbotfn.tk/api/v1/cosmetics/br/search/all?lang=en&searchLang=en&matchMethod=full&name=%s&backendType=AthenaGlider", selection);
    ...
    public static void processGlider() {
        try {
            // Glider to Mesh to Material
            ...
            MeshesList.add(cpStructure[0].SkeletalMesh.asset_path_name.split("\\.")[0]);

            if (toJson.contains("OverrideMaterial")) {
                for (int j : range(cpStructure[0].MaterialOverrides.length)) {
                    MaterialsList.add(new overrideStructure3D(cpStructure[0].SkeletalMesh.asset_path_name.split("\\.")[1],
                            cpStructure[0].MaterialOverrides[j].OverrideMaterial.asset_path_name.split("\\.")[0],
                            cpStructure[0].MaterialOverrides[j].MaterialOverrideIndex,
                            "true"));
                }
            } else {
                pkg = (IoPackage) fileProvider.loadGameFile(cpStructure[0].SkeletalMesh.asset_path_name.split("\\.")[0] + ".uasset");
